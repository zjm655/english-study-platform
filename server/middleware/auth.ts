...
export default defineEventHandler(async (event) => {
  ...
  // 4. 校验用户是否仍存在，并以 DB 的 role 为权威（防止旧 token 指向已删除用户，或降权后持旧 token 越权）。
  //    主键索引查询，开销极小。
  const rows = await query<{ id: number; role: number }>('SELECT id, role FROM user WHERE id = ? LIMIT 1', [payload.id])
  if (rows.length === 0) {
    deleteCookie(event, 'token')
