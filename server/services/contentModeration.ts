...
3. 内容必须是英文，或至少以英文为主——这是用于语言学习的英文材料。如果内容是中文或其他语言，或带时间轴/序号/字幕(SRT)等格式，视为不合规
4. 内容应该是适合语言学习的正常英语材料（个人独白、趣味文章、新闻报道等非对话类文本；对话文本仅在明确提示允许时可用）
 ...
 * 无需担心「无法区分多角色」问题，对话文本应可正常使用；英文/非字幕规则完全一致）。
 */
const SYSTEM_PROMPT_ALLOW_DIALOGUE = SYSTEM_PROMPT.replace(
  '4. 内容应该是适合语言学习的正常英语材料（个人独白、趣味文章、新闻报道等非对话类文本；对话文本仅在明确提示允许时可用）',
  '4. 内容可以是个人独白、趣味文章、新闻报道或对话形式（对话需为正常的语言学习材料）',
)
